  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full w-[250px] bg-sidebar text-white flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4">
          <button onClick={handleLogout} className="cursor-pointer flex items-center gap-3 px-4 py-2.5 text-sm text-gray-400 hover:text-white w-full rounded-lg hover:bg-sidebar-light transition-colors">
            <LogOut size={18} />
            <span>Log out</span>
          </button>
        </div>
