...
import {
  Users, UserCheck, Clock, FileText, Percent, DollarSign,
  Timer, CalendarOff, Calendar, TrendingUp, Building2, ChevronRight
} from 'lucide-react';
import {
...
export default function AdminDashboard() {
  ...
  const attendanceChartData = chartView === '7days'
    ? (stats.attendanceLast7Days || []).map(d => ({
        name: new Date(d._id).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }),
        Present: d.present,
        Late: d.late,
        Absent: d.absent
      }))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <StatCard title="Present Today" value={stats.todayPresent ?? 0} icon={UserCheck} color="success" />
        <StatCard title="Late Today" value={stats.todayLate ?? 0} icon={Clock} color="warning" />
        <StatCard title="Pending Leaves" value={stats.pendingLeaves ?? 0} icon={FileText} color="danger" />
      </div>

      {/* Row 2: Secondary Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard title="Attendance Rate" value={`${stats.attendanceRate ?? 0}%`} icon={Percent} color="success" subtitle="Today" />
        <StatCard title="Payroll This Month" value={stats.payrollThisMonth ? `৳${stats.payrollThisMonth.grossSalary?.toLocaleString() || 0}` : '—'} icon={DollarSign} color="primary" subtitle={stats.payrollThisMonth?.status || 'No payroll'} />
        <StatCard title="Overtime" value={`${stats.overtimeHours ?? 0}h`} icon={Timer} color="warning" subtitle="Today" />
        <StatCard title="On Leave Today" value={stats.onLeaveToday ?? 0} icon={CalendarOff} color="danger" subtitle="Approved leaves" />
      </div>
      ...
      <SectionCard title="Attendance Overview" className="mb-6">
        ...
        {attendanceChartData.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-64 text-text-secondary text-sm">No attendance data available</div>
        )}
      </SectionCard>
      ...
      <SectionCard title="Department Overview" className="mb-6">
        {departmentData.length > 0 ? (
            ...
            {departmentData.map(dept => (
              <div key={dept.name}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-text-secondary">{dept.attendanceRate}%</span>
                </div>
                <div className="w-full h-2.5 bg-page-bg rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      dept.attendanceRate >= 90 ? 'bg-success' :
                      dept.attendanceRate >= 70 ? 'bg-warning' : 'bg-danger'
                    }`}
          ...
          <div className="text-center text-text-secondary text-sm py-8">No department data available</div>
        )}
      </SectionCard>
        ...
        <SectionCard title="Leave Requests">
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              ...
              <span className="text-lg font-bold">{stats.leaveRequests?.pending || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold">{stats.leaveRequests?.approved || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold">{stats.leaveRequests?.rejected || 0}</span>
            </div>
          </div>
        ...
        <SectionCard title="Today's Attendance">
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              ...
              <span className="text-lg font-bold">{stats.todayPresent || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-warning" />
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
              ...
              <span className="text-lg font-bold">{stats.todayAbsent || 0}</span>
            </div>
          </div>
          ...
          {stats.payrollThisMonth ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
                ...
                <span className="text-sm font-bold">৳{stats.payrollThisMonth.grossSalary?.toLocaleString() || 0}</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
                <span className="text-sm text-text-secondary">Deductions</span>
                <span className="text-sm font-bold text-danger">- ৳{stats.payrollThisMonth.deductions?.toLocaleString() || 0}</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-primary/5 border border-primary/20">
                <span className="text-sm font-semibold">Net Salary</span>
                <span className="text-sm font-bold text-primary">৳{stats.payrollThisMonth.netSalary?.toLocaleString() || 0}</span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-page-bg">
                <span className="text-sm text-text-secondary">Status</span>
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
                  stats.payrollThisMonth.status === 'PAID' ? 'bg-success/10 text-success' :
                  stats.payrollThisMonth.status === 'PROCESSED' ? 'bg-primary/10 text-primary' :
        ...
        <SectionCard title="Employee Growth">
          {stats.employeeGrowth?.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={stats.employeeGrowth} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                <XAxis dataKey="month" tick={{ fontSize: 11 }} />
                <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #e5e7eb' }} />
                <Area type="monotone" dataKey="employees" stroke="#5046e5" fill="#5046e5" fillOpacity={0.15} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="text-center text-text-secondary text-sm py-8">No growth data available</div>
          )}
          <div className="flex items-center justify-around mt-3 pt-3 border-t border-border">
            <div className="text-center">
              <div className="text-lg font-bold text-success">{stats.activeEmployees ?? 0}</div>
              <div className="text-[11px] text-text-secondary">Active</div>
            </div>
            <div className="w-px h-8 bg-border" />
            <div className="text-center">
              <div className="text-lg font-bold text-danger">{stats.inactiveEmployees ?? 0}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard title="Recent Employees">
          {stats.recentEmployees?.length > 0 ? (
              ...
              {stats.recentEmployees.map(emp => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold truncate">{emp.firstName} {emp.lastName}</div>
                    <div className="text-[11px] text-text-secondary truncate">{emp.position} · {emp.department}</div>
                  </div>
            ...
            <div className="text-center text-text-secondary text-sm py-8">No recent employees</div>
          )}
        </SectionCard>
          ...
          {stats.upcomingHolidays?.length > 0 ? (
            <div className="space-y-3">
              {stats.upcomingHolidays.map((holiday, index) => (
                <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-page-bg">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-text-secondary">
                      ...
                      {holiday.startDate !== holiday.endDate && (
                        <> — {new Date(holiday.endDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="text-center text-text-secondary text-sm py-8">No upcoming holidays</div>
          )}
        </SectionCard>
      </div>
    </div>
