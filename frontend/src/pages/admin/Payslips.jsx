...
import { Plus, Download, Pencil, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AdminPayslips() {
  ...
  const fetchPayslips = () => {
    ...
    api.get(`/get-payslips?${params.toString()}`).then(res => {
      setPayslips(res.data.data || []);
      setPagination(res.data.pagination || { currentPage: 1, totalPages: 1, totalCount: 0, limit: 10 });
    }).catch(() => {});
  };

  useEffect(() => { fetchPayslips(); }, [page, search]);
  ...
  return (
    <div>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
          <div className="text-sm text-text-secondary">
            Showing {((pagination.currentPage - 1) * pagination.limit) + 1} to {Math.min(pagination.currentPage * pagination.limit, pagination.totalCount)} of {pagination.totalCount} payslips
          </div>
          <div className="flex items-center gap-2">
            <button
      ...
      <EditPayslipModal isOpen={!!editPayslip} onClose={() => setEditPayslip(null)} onSuccess={fetchPayslips} payslip={editPayslip} />
    </div>
  );
}