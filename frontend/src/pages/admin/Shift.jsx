...
export default function AdminShift() {
  ...
  const fetchPublicHolidays = () => {
    api
      .get("/public-holidays")
      .then((res) => setPublicHolidays(res.data.data || []))
      .catch(() => {});
  };
  ...
  const handleHolidaySave = async () => {
    try {
      if (editingHoliday)
        await api.patch(`/public-holidays/${editingHoliday._id}`, holidayForm);
      else await api.post("/public-holidays", holidayForm);
  ...
  const handleShiftSave = async () => {
    try {
      if (editingShift)
        await api.patch(`/update-shift/${editingShift._id}`, shiftForm);
      else await api.post("/create-shift", shiftForm);
  ...
  const handleShiftActivate = async (id) => {
    try {
      await api.patch(`/shift/${id}/activate`);
      fetchShifts();
    } catch (err) {
      alert(getApiError(err, "Failed"));
    }
  ...
  const handleAssign = async () => {
    try {
      if (assignMode === "single") {
        if (!selectedEmployee || !selectedShift || !assignDate) {
          alert("Please select employee, shift and date");
          return;
        }
        await api.post("/assign-shift", {
        ...
        const res = await api.post("/assign-shift-month", {
          employeeId: selectedEmployee,
          shiftId: selectedShift,
          month: Number(assignMonth),
  ...
  const fetchViewAssignments = () => {
    ...
    api
      .get(
        `/employee-roster/${viewEmployee}?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`,
      )
      .then((res) => setViewAssignments(res.data.data || []))
      .catch(() => setViewAssignments([]));
  };
  useEffect(() => {
    fetchViewAssignments();
  }, [viewEmployee, viewMonth, viewYear]);

  const handleUpdateAssignment = async () => {
    if (!editingAssignment || !editShiftId) return;
    try {
      await api.patch(`/update-shift-assignment/${editingAssignment._id}`, {
        shiftId: editShiftId,
      });
  ...
  const viewEmployeeOptions = employees.map((e) => ({
    value: e._id,
  ...
  return (
    <div>
      ...
      <p className="text-text-secondary text-sm mb-6">
        Manage shifts and assign them to employees
      </p>

      <div className="flex gap-2 mb-6 border-b border-border overflow-x-auto">
        {[
          { key: "shifts", label: "Shifts" },
          { key: "assign", label: "Assign Shift" },
                  ...
                  {filteredShifts.map((shift) => (
                    ...
                    >
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        <span
                          className={`text-xs px-3 py-1 rounded-full font-medium ${shift.isActive ? "bg-green-50 text-green-600" : "bg-amber-50 text-amber-600"}`}
                        >
                      ...
                      <td className="px-4 sm:px-6 py-4">
                        <div className="flex gap-2">
                          ...
                          {shift.isActive ? (
                            <button
                              onClick={() => handleShiftDelete(shift._id)}
                              className="text-xs bg-amber-50 text-amber-600 px-3 py-1 rounded-full hover:bg-red-100"
                            >
                              Deactivate
                            </button>
                          ) : (
                            <button
                              onClick={() => handleShiftActivate(shift._id)}
          ...
          {viewEmployee && (
            <div className="bg-card border border-border rounded-xl p-4 sm:p-6 mb-6">
              ...
              <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden">
                ...
                {Array.from({
                  length: getFirstDayOfWeek(viewMonth, viewYear),
                }).map((_, i) => (
                  <div
                    key={`empty-${i}`}
                ...
                {Array.from({
                  length: getDaysInMonth(viewMonth, viewYear),
                }).map((_, i) => {
                  const day = i + 1;
                  ...
                  const isWeekend =
                    assignment?.shift?.weekends?.includes(dayOfWeek);
                  return (
                    <div
                      key={day}
                          ...
                          <td className="px-4 sm:px-6 py-4 text-sm font-medium">
                            {editingAssignment?._id === s._id ? (
                              <select
                                ...
                                className="px-2 py-1 border border-border rounded text-sm focus:outline-none focus:border-primary"
                              >
                                {activeShifts.map((sh) => (
                              ...
                              {editingAssignment?._id === s._id ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full hover:bg-gray-100"
                                  >
                                    Cancel
                                  </button>
                                </>
                  ...
                  {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                    (day, i) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          const weekends = shiftForm.weekends.includes(i)
                            ? shiftForm.weekends.filter((d) => d !== i)
                            : [...shiftForm.weekends, i];
                          setShiftForm({ ...shiftForm, weekends });
                        }}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${shiftForm.weekends.includes(i) ? "bg-amber-50 border-red-300 text-amber-600" : "border-border text-text-secondary hover:bg-page-bg"}`}
                      >
                        {day}
                      </button>
                    ),
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-text-secondary mt-1">
                  Same date for single-day holiday
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
