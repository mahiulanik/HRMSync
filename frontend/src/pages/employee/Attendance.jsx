...
export default function EmployeeAttendance() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      if (todayRecord) {
        setClockedIn(Boolean(todayRecord.checkIn && !todayRecord.checkOut));
      } else {
        setClockedIn(false);
      }
      ...
      const workDays = data.filter((a) => a.workingHours > 0).length;
  ...
  return (
    <div>
          ...
          <table className="w-full min-w-[550px]">
            ...
            <tbody>
              {attendance.map((a) => (
                <tr
                  key={a._id}
                  className={`border-b border-border last:border-0 ${a.status === "ABSENT" ? "bg-red-50/50" : a.status === "WEEKEND" ? "bg-amber-50/50" : ""}`}
                >
                  ...
                  <td className="px-4 sm:px-6 py-4 text-sm">
                    {a.workingHours
                      ? `${Math.floor(a.workingHours)}h ${Math.round((a.workingHours % 1) * 60)}m`
      ...
      {message.text && (
        <div
          className={`fixed bottom-24 right-4 sm:right-8 px-4 py-2.5 rounded-lg text-sm font-medium shadow-lg z-50 ${message.type === "success" ? "bg-green-50 text-green-600 border border-green-200" : message.type === "error" ? "bg-red-50 text-red-600 border border-red-200" : "bg-blue-50 text-blue-600 border border-blue-200"}`}
        >
          {message.text}
        </div>
      )}
