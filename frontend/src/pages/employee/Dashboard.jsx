...
export default function EmployeeDashboard() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    let status =
      statusMap[dateKey] || (isFuture ? "future" : isWeekend ? null : "ABSENT");
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={
            stats.latestPayslip
              ? `৳${stats.latestPayslip.netSalary?.toLocaleString() || 0}`
              : "৳0"
          }
          icon={DollarSign}
          color="primary"
          ...
          stats.todayShift.isWeekend ? (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full ${stats.clockedIn ? "bg-success" : "bg-text-secondary"}`}
                  />
      ...
      <SectionCard title="📅 Attendance Calendar" className="mb-6">
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div
              key={d}
              className="text-center text-[10px] font-medium text-text-secondary py-1"
            >
              {d}
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <SectionCard title="🏖 Leave Balance">
          <div className="space-y-3">
            {leaveTypes.map((lt) => {
              const balance = stats.leaveBalance?.[lt.key] || {
                used: 0,
                total: 0,
              };
              ...
              return (
                <div key={lt.key} className="p-3 rounded-lg bg-page-bg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-bold ${lt.color}`}>
