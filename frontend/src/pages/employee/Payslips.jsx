...
export default function EmployeePayslips() {
  ...
  useEffect(() => { api.get('/get-payslips').then(res => setPayslips(res.data.data || [])).catch(() => {}); }, []);

  const handleDownload = async (payslipId) => {
    try {
      const res = await api.get(`/get-payslip-by-id/${payslipId}`);
      const p = res.data.data; const emp = p.employee || p.employeeId || {};
  ...
  return (
    <div>
      ...
      <p className="text-text-secondary text-sm mb-6">Your payslip history</p>
          ...
          <table className="w-full min-w-[400px]">
            ...
            <tbody>{payslips.map(p => (
              <tr key={p._id} className="border-b border-border last:border-0">
                <td className="px-4 sm:px-6 py-4 text-sm">{MONTH_NAMES[p.month - 1]} {p.year}</td>
            ...
            ))}{payslips.length === 0 && <tr><td colSpan={4} className="px-6 py-12 text-center text-text-secondary">No payslips found</td></tr>}</tbody>
          </table>
        </div>
      </div>
    </div>
