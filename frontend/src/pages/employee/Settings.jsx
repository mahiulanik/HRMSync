...
export default function EmployeeSettings() {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        if (d.profilePic) setPreview(d.profilePic);
      })
      .catch(() => {});
  }, []);
  ...
  const handleFileSelect = (e) => {
    ...
    if (file.size > 300 * 1024) {
      setUploadMsg("Image must be less than 100KB");
      e.target.value = "";
      return;
    }
    const allowedTypes = ["image/jpeg", "image/png", "image/webp", "image/gif"];
    if (!allowedTypes.includes(file.type)) {
      setUploadMsg("Only JPG, PNG, WebP and GIF images are allowed");
      e.target.value = "";
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await api.post("/profile/pic", formData, {
      ...
      setUploadMsg(getApiError(err, "Failed to upload"));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleProfileSave = async () => {
    setMsg("");
    try {
      await api.post("/profile", { bio: profile.bio });
      setMsg("Profile updated successfully");
    } catch (err) {
      setMsg(getApiError(err, "Failed to update profile"));
    }
  };
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      setPwMsg("Password changed successfully. Redirecting to login...");
      setTimeout(async () => {
        await logout();
        navigate("/", { replace: true });
      }, 2000);
    } catch (err) {
      setPwMsg(getApiError(err, "Failed to change password"));
    }
  };
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-text-secondary mt-2">
              JPG, JPEG, PNG, WebP or GIF. Max 300KB.
            </p>
            {uploadMsg && (
              <p
                className={`text-xs mt-2 ${uploadMsg.includes("success") ? "text-green-600" : "text-red-600"}`}
              >
                {uploadMsg}
              </p>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              ...
              readOnly
              className="w-full px-3 py-2.5 border border-border rounded-lg bg-page-bg text-text-secondary"
            />
          </div>
          <div>
      ...
      >
        ...
        <form onSubmit={handlePasswordChange}>
          ...
          {pwMsg && (
            <p
              className={`text-sm mb-3 ${pwMsg.includes("success") ? "text-green-600" : "text-red-600"}`}
            >
              {pwMsg}
            </p>
          )}
